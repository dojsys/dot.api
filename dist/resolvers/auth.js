...
const crypto = require('crypto'); // resolve the `AuthPayload` type
...
async function sendLinkValidateEmail(parent, args, ctx, info) {
  const id = getUserId(ctx);
  let userMe = await ctx.db.query.user({
    where: {
      id
    }
  });
  return emailGenerator.sendWelcomeEmail(userMe, ctx).then(data => {
    return userMe;
  }).catch(data => {
...
async function forgetPassword(parent, {
  email
}, ctx, info) {
  const user = await ctx.db.query.user({
    where: {
...
async function updatePassword(parent, {
  oldPassword,
  newPassword
}, ctx, info) {
